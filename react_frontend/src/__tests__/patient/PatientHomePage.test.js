...
describe('PatientHomePage', () => {
  ...
  test('renders menu items and redirects correctly', () => {
    const homeMenuItem = screen.getByTestId('link-/patient-home-page').querySelector('span');
    expect(homeMenuItem).toHaveTextContent('Home');
  
    const reportsLink = screen.getByTestId('link-/patient-reporting-landing-page').querySelector('span');
    expect(reportsLink).toHaveTextContent('My Reports');
  
    const supportLink = screen.getByTestId('link-/support-page').querySelector('span');
  ...
  test('renders correct date in EST', () => {
    ...
    const ordinalSuffix = (day > 3 && day < 21) ? 'th' : ['st', 'nd', 'rd'][((day % 10) - 1) % 3] || 'th';
    const expectedDate = currentDate.replace(day, `${day}${ordinalSuffix}`);

    const dateElement = screen.getByText(new RegExp(`Today is ${expectedDate}`, 'i'));
